...
function animatePercentRing(ring,label,to,duration=1100){if(!ring||!label)return;const from=Number(ring.dataset.percent||0);const target=Math.min(Math.max(Number(to)||0,0),100);ring.dataset.percent=target;animateValue(ring,from,target,duration,v=>{ring.style.setProperty("--p",v+"%");label.textContent=v.toFixed(0)+"%"})}
function renderFireJourney(t,f){const goal=raw(state?.fire?.goal)||0;const current=t.netWorth;const remaining=Math.max(goal-current,0);const pct=Math.min(Math.max(f.percent||0,0),100);if($("journeyGoal"))$("journeyGoal").textContent=fmt(goal);animateMoney("journeyCurrent",current,980);animateMoney("journeyRemain",remaining,980);animatePercentRing($("journeyRing"),$("journeyPercent"),pct,1250);if($("journeyEta")){if(f.months===null)$("journeyEta").textContent="100 年以上";else{const d=new Date();d.setMonth(d.getMonth()+f.months);$("journeyEta").textContent=`${d.getFullYear()} / ${String(d.getMonth()+1).padStart(2,"0")}`}}if($("journeyYears"))$("journeyYears").textContent=f.months===null?"--":`${(f.months/12).toFixed(1)} 年`;const card=document.querySelector(".fire-journey-card");if(card){card.classList.remove("motion-ready");void card.offsetWidth;card.classList.add("motion-ready")}}
