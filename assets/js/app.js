...
const fireQuotes = ["財務的累積，需要時間。","今天的小累積，成就明天的自由。","真正的財富，是時間的複利。","持續前進，比快速前進更重要。","每一次投資，都是未來的自己。","離 FIRE，又近了一點。","今天，也為未來的自己努力。","市場會波動，紀律不該波動。","每一筆投資，都在替未來工作。","你不是在存錢，而是在購買未來的自由。","時間，是最強大的投資工具。","今天的選擇，決定十年後的生活。"];
function setDailyQuote(){ const el = $("greeting"); if(!el) return; const idx = Math.floor(Date.now() / 86400000) % fireQuotes.length; el.textContent = fireQuotes[idx]; }
const fieldConfigs = {
...
const defaultData = {
  version:"2.5.2", settings:{displayName:"", emergencyMonths:6}, fire:{goal:30000000, monthlyInvestment:60000, annualReturn:8},
  ...
  journal:[{id:uid(),date:today(),title:"建立 FIRE OS 2.5.2 Stable",amount:0,note:"穩定版：Dashboard 改為首頁，新增首頁每日語錄。"}],
...
function normalizeState(data){const base=clone(defaultData);const merged={...base,...(data||{})};["assets","liabilities","investments","income","expenses","journal","history"].forEach(k=>{if(!Array.isArray(merged[k]))merged[k]=[];merged[k]=merged[k].map(item=>({id:item.id||uid(),...item}))});merged.settings={...base.settings,...(merged.settings||{})};merged.fire={...base.fire,...(merged.fire||{})};merged.version="2.3";return merged}
async function loadData(){const snap=await getDoc(userRef());if(snap.exists())state=normalizeState(snap.data());else{state=clone(defaultData);state.profile={uid:user.uid,email:user.email,name:user.displayName,photo:user.photoURL};await saveData()}renderAll()}
...
function renderAll(){if(!state)return;setDailyQuote();renderUser();renderDashboard();renderLists();renderFire();renderAI();renderCharts()}
...
function blockMobileLineLogin(){
  alert("手機版 LINE 登入開發中，請先使用 Google 登入。\n\n桌機版仍可使用 LINE Login。");
}
function applyMobileLineSafety(){
  ...
  if (lineLoginBtn && isMobile) {
    lineLoginBtn.textContent = "LINE 登入（手機版開發中）";
    lineLoginBtn.classList.add("disabled-line");
    lineLoginBtn.setAttribute("aria-disabled", "true");
  }
